...
def generate_salary_slips_for_specific_month():
    ...
    try:
        ...
        print(f"Starting salary slip generation for {len(active_employees)} employees for {target_date}")
        ...
        for employee in active_employees:
            try:
                ...
                existing_slip = frappe.db.exists(
                    ...
                    {
                        "employee": employee_id,
                        "start_date": [">=", frappe.utils.get_first_day(target_datetime)],
                        "end_date": ["<=", frappe.utils.get_last_day(target_datetime)],
                        "workflow_state": ["not in", ["Pending"]]
                    }
                )
                ...
                if draft_slip:
                    ...
                    salary_slip = frappe.new_doc("Salary Slip")
                    salary_slip.employee = employee_id
                    salary_slip.posting_date = target_date
                    salary_slip.start_date = frappe.utils.get_first_day(target_datetime)
                    salary_slip.end_date = frappe.utils.get_last_day(target_datetime)
                    salary_slip.company = frappe.db.get_single_value("Global Defaults", "default_company")
